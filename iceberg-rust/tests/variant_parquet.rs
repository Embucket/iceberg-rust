...
fn parquet_variant_roundtrip_preserves_values_and_logical_type() -> Result<(), Box<dyn Error>> {
    ...
    let output_variant = VariantArray::try_new(output.column(0).as_ref())?;
    let input = input
        .as_any()
        .downcast_ref::<StringArray>()
        .ok_or("expected string input")?;
    for row in [0, 1] {
        let expected: serde_json::Value = serde_json::from_str(input.value(row))?;
        let actual: serde_json::Value =
            serde_json::from_str(&output_variant.value(row).to_json_string()?)?;
        assert_eq!(actual, expected);
    }
    assert!(output_variant.is_null(2));
    Ok(())
}
